...
public class FinalLevelUnlock : MonoBehaviour
{
    ...
    {   
        ...
        else
        {
            ...
            playerManagerScript.setUnlockedBonus(newUnlock);
    ...
    private void Update() 
    {
        ...
        if(playerManagerScript.getUnlockedBonus() == newUnlock && !isDeactivated)
        {
            disableButton();
            isBought = true;
        }
        //Change check per script
        else if(playerManagerScript.getUnlockedBonus() == reqUnlock && isDeactivated)
        {
            enableButton();
        }
